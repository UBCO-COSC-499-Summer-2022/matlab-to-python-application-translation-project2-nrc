...
class SequenceSelector:
	...
	def __init__(self, master, title, command=None):
		...
		self.entry = tk.Entry(self.top_frame, textvariable=self.entry_var,
							  state=tk.DISABLED, validate="focusout", validatecommand=self.validate_entry, width=3)
		self.length_label = tk.Label(self.top_frame, text=" / 0")
		self.scale = tk.Scale(
			self.bottom_frame, from_=1, state=tk.DISABLED, orient=tk.HORIZONTAL, command=self.handle_scale)
		self.left_button = tk.Button(self.bottom_frame, text="<", command=self.handle_left_button)
		self.right_button = tk.Button(self.bottom_frame, text=">", command=self.handle_right_button)
